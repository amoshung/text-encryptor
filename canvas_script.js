...
function convertDateFormat(text) {
  const numberToChineseMap = {
    ...
    6: "六", 7: "七", 8: "八", 9: "九", 10: "十", 11: "十一", 12: "十二"
  };
...
function transformNewsTitle() {
  ...
  // 隨機選擇開始和結束關鍵詞
  const randomStartIndex = Math.floor(Math.random() * shocking_title_keywords_restructured.startKeyword.length);
  const randomEndIndex = Math.floor(Math.random() * shocking_title_keywords_restructured.endKeyword.length);
  
  // 獲取關鍵詞並確保其為純文本
  let startKeyword = shocking_title_keywords_restructured.startKeyword[randomStartIndex];
  let endKeyword = shocking_title_keywords_restructured.endKeyword[randomEndIndex];
...
function transformNewsTitlePositive() {
  ...
  // 隨機選擇開始和結束關鍵詞
  const randomStartIndex = Math.floor(Math.random() * shocking_title_keywords_restructured_GOOD.startKeyword.length);
  const randomEndIndex = Math.floor(Math.random() * shocking_title_keywords_restructured_GOOD.endKeyword.length);
  
  // 獲取關鍵詞並確保其為純文本
  let startKeyword = shocking_title_keywords_restructured_GOOD.startKeyword[randomStartIndex];
  let endKeyword = shocking_title_keywords_restructured_GOOD.endKeyword[randomEndIndex];
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (titleWidthSelect) {
    ...
    // 初始設置
    document.documentElement.style.setProperty('--title-width', titleWidthSelect.value + "%");
  }
...
function initializeCanvas() {
  // 檢查是否已存在標題框
  const existingTitleBox = document.querySelector(".text-box-wrapper[data-id='0']");
...
function initializeFontSizeListeners() {
  ...
  if (charsizeInput) {
    ...
    // 直接給所有正文文字框設置字體大小
      const textBoxes = document.querySelectorAll(".text-box-wrapper:not([data-id='0'])");
...
function autoResizeTextBox(textElement) {
  ...
  if (wrapper.dataset.id === "0") {
    ...
    // 普通文字框
    // 計算可用空間，考慮最小邊距
    const maxWidth = canvasRect.width - (wrapperRect.left - canvasRect.left) - minMargin;
    const maxHeight = canvasRect.height - (wrapperRect.top - canvasRect.top) - minMargin;
    ...
    const currentLeft = parseFloat(wrapper.style.left) || wrapperRect.left - canvasRect.left;
    const currentTop = parseFloat(wrapper.style.top) || wrapperRect.top - canvasRect.top;
    ...
    if (rightEdge > canvasRect.width - minMargin) {
      wrapper.style.left = `${canvasRect.width - wrapper.offsetWidth - minMargin}px`;
    }
    
    // 確保下邊距
    const bottomEdge = currentTop + wrapper.offsetHeight;
    if (bottomEdge > canvasRect.height - minMargin) {
      wrapper.style.top = `${canvasRect.height - wrapper.offsetHeight - minMargin}px`;
    }
  }
...
function lockCanvas() {
  ...
  textBoxes.forEach(box => {
    ...
    // 5. 隱藏控制按鈕和縮放按鈕
    const controls = box.querySelector("div[style*='position: absolute'][style*='right: -40px']");
    if (controls) {
      controls.style.display = "none";
    }
    
    const resizeBtn = box.querySelector("div[style*='position: absolute'][style*='bottom: 0'][style*='right: 0']");
    if (resizeBtn) {
...
function connectTitleFunctions() {
  ...
  // 獲取標題框
  const titleBox = document.querySelector(".text-box-wrapper[data-id='0'] .text-content");
